...
class PDF extends FPDF
	...
	function PrintScoreSheet($seasonname,$gameId,$hometeamname,$visitorteamname,$poolname,$time,$placename)
		{
		$this->game['seasonname'] = utf8_to_cp1251($seasonname);
		$this->game['game_id'] = $gameId."".getChkNum($gameId);
		$this->game['hometeamname'] = utf8_to_cp1251($hometeamname);
		$this->game['visitorteamname'] = utf8_to_cp1251($visitorteamname);
		$this->game['poolname'] = utf8_to_cp1251($poolname);
		$this->game['time'] = $time;
		$this->game['placename'] = utf8_to_cp1251($placename);
		...
		$data .= " - ";
		$data .= _("Game Record"); 
		$data = utf8_to_cp1251($data); //season name already decoded
		...
		$this->OneCellTable(utf8_to_cp1251(_("Game #")), $this->game['game_id']);
		$this->OneCellTable(utf8_to_cp1251(_("Home team")), $this->game['hometeamname']);
		$this->OneCellTable(utf8_to_cp1251(_("Away team")), $this->game['visitorteamname']);
		$this->OneCellTable(utf8_to_cp1251(_("Division").", "._("Pool")), $this->game['poolname']);
		$this->OneCellTable(utf8_to_cp1251(_("Field")), $this->game['placename']);
		$this->OneCellTable(utf8_to_cp1251(_("Scheduled start date and time")), $this->game['time']);
		$this->OneCellTable(utf8_to_cp1251(_("Game official")), "");
		...
		$this->OneCellTable(utf8_to_cp1251(_("Half time ends")), "");
		$this->Ln();
		$this->SpiritPoints();
		$this->Ln();
	...
	function PrintPlayerList($homeplayers, $visitorplayers)
		...
		$this->Cell(56,6,utf8_to_cp1251(_("Name")),'LRTB',0,'C',true);
		$this->Cell(15,6,utf8_to_cp1251(_("Jersey#")),'LRTB',0,'C',true);
		$this->Cell(23,6,utf8_to_cp1251(_("Info")),'LRTB',0,'C',true);
		...
		for($i=1;$i<31;$i++)
			...
			if(isset($homeplayers[$i-1]['name'])){
				$hplayer = utf8_to_cp1251($homeplayers[$i-1]['name']);
				$hnumber = $homeplayers[$i-1]['num'];
			}
			if(isset($visitorplayers[$i-1]['name'])){
				$vplayer = utf8_to_cp1251($visitorplayers[$i-1]['name']);
  ...
  function PrintRoster($teamname, $seriesname, $poolname, $players) {
		...
		$this->SetFont('Times','',10);
		$this->Cell(8,6,"",'LRTB',0,'C',true);
		$this->Cell(100,6,utf8_to_cp1251(_("Name")),'LRTB',0,'C',true);
		$this->Cell(14,6,utf8_to_cp1251(_("Play")),'LRTB',0,'C',true);
		$this->Cell(13,6,utf8_to_cp1251(_("Game#")),'LRTB',0,'C',true);
		$this->Cell(55,6,utf8_to_cp1251(_("Info")),'LRTB',0,'C',true);
		$this->Ln();
		$this->SetTextColor(0);
		$this->SetFillColor(255);
		for($i=1;$i<26;$i++){
			...
			if(isset($players[$i-1]['firstname'])){
				$player .= utf8_to_cp1251($players[$i-1]['firstname']);
			}
		    $player .= " ";
			if(isset($players[$i-1]['lastname'])){
				$player .= utf8_to_cp1251($players[$i-1]['lastname']);
			}
			...
			$this->SetFont('Times','',10);
			$this->Cell(14,6,"",'LRTB',0,'C',true);
			if(isset($players[$i-1]['num']) && $players[$i-1]['num']>=0){
			  $this->Cell(13,6,$players[$i-1]['num'],'LRTB',0,'C',true);
			}else{
			  $this->Cell(13,6,"",'LRTB',0,'C',true);
			}
			$this->Cell(55,6,"",'LRTB',0,'C',true);
	...
	function PrintSchedule($scope, $id, $games)
		...
		while($game = mysql_fetch_assoc($games)){
			...
			if(!empty($game['place_id']) && $game['reservationgroup'] != $prevTournament) {
				$txt = utf8_to_cp1251(U_($game['reservationgroup']));
				$this->SetFont('Times','B',12);
				$this->SetTextColor(0);
				$this->Ln();
				$this->Write(5, $txt);
	...
	function PrintOnePageSchedule($scope, $id, $games, $colors=false){
		...
		while($game = mysql_fetch_assoc($games)){
			...
			if(!empty($game['place_id']) && $game['reservationgroup'] != $prevTournament || $prevDate != JustDate($game['starttime'])) {
				...
				$title = utf8_to_cp1251(SeasonName($id));
				$title .= " ".utf8_to_cp1251($game['reservationgroup']);
				$title .= " (".utf8_to_cp1251(ShortDate($game['starttime'])).")";
			...
			if(!empty($game['place_id']) && $game['fieldname'] != $prevField){
				...
				$txt = utf8_to_cp1251(_("Field")." ".$game['fieldname']);
				$this->Cell($gridx,$yfieldtitle/2,$txt,"LRT",2,'C',true);
				
				$this->SetFont('Times','',8);
				$this->SetTextColor(0);
				$txt = utf8_to_cp1251($game['placename']);
			...
			if($game['hometeam'] && $game['visitorteam']){
				...
				$txt = utf8_to_cp1251($game['visitorteamname']);
			...
			$txt = utf8_to_cp1251($game['seriesname']);
			if(strlen($game['poolname'])<15){
				$txt .= ", \n";
			}else{
				$txt .= ", ";
			}
			$txt .= utf8_to_cp1251($game['poolname']);
	...
	function DynSetTeamName($longname, $abbrev, $x, $fontsize){
		...
		$text = utf8_to_cp1251($longname);
		if($this->GetStringWidth($text)>$x-2 && !empty($abbrev)){
			$text = utf8_to_cp1251($abbrev);
		}
	...
	function GameRowWithPool($game, $date=false, $time=true, $field=true, $pool=true, $result=true) {
		...
		if($field){
			$txt = utf8_to_cp1251(U_($info['fieldname']));
		...
		if($game['gamename']){
			...
			$txt = utf8_to_cp1251(U_($game['gamename']).":");
		...
		if($game['hometeam'] && $game['visitorteam']){
			$txt = utf8_to_cp1251($game['hometeamname']);
			...
			$txt = utf8_to_cp1251($game['phometeamname']);
			$this->Cell(45-$o,5,$txt,'TB',0,'L',true);
			$txt = " - ";
			$this->Cell(5,5,$txt,'TB',0,'L',true);
			$txt = utf8_to_cp1251($game['pvisitorteamname']);
			$this->Cell(45-$o,5,$txt,'TB',0,'L',true);
			$this->SetFont('Times','',8);
		}
		if($pool){
			$txt = utf8_to_cp1251(U_($game['seriesname']));
			$this->Cell(20,5,$txt,'TB',0,'L',true);
			
			$txt = utf8_to_cp1251(U_($game['poolname']));
	...
	function PrintSeasonPools($id) {
		...
		foreach($series as $row){
			...
			$name = utf8_to_cp1251(U_($row['name']));
	...
	function PrintSeriesPools($id) {
		...
		$name = utf8_to_cp1251(U_(SeriesName($id)));
		$this->SetFont('Times','B',14);
		$this->SetTextColor(0);
	...
	function PrintPools($pools) {
		...
		foreach ($pools as $pool) {
			...
			$name = utf8_to_cp1251(U_($poolinfo['name']));
			...
			foreach($teams as $team){
				$txt = utf8_to_cp1251(U_($team['name']));
				$fontsize=9;
				if($scheduling_teams){
					$this->SetFont('Times','i',$fontsize);
				}else{
					$this->SetFont('Times','',$fontsize);
				}
	...
	function Timeouts()
		...
		$this->SetTextColor(255);
		$this->SetFillColor(0,102,153);
		$this->Cell(80,6,utf8_to_cp1251(_("Timeouts")),'LRTB',0,'C',true);
		$this->Ln();
		
		//home grids
		$this->SetTextColor(0);
		$this->SetFillColor(255);
		$this->Cell(20,6,utf8_to_cp1251(_("Home team")),'LRTB',0,'L',true);
		...
		$this->SetTextColor(0);
		$this->SetFillColor(255);
		$this->Cell(20,6,utf8_to_cp1251(_("Away")),'LRTB',0,'L',true);
	...
	function FirstOffence()
		...
		$this->SetTextColor(255);
		$this->SetFillColor(0,102,153);
		$this->Cell(80,6,utf8_to_cp1251(_("Starting offensive team")),'LRTB',0,'C',true);
		$this->Ln();
	...
	function SpiritPoints()
		...
		$this->SetTextColor(255);
		$this->SetFillColor(0,102,153);
		$this->Cell(80,6,utf8_to_cp1251(_("Spirit points")),'LRTB',0,'C',true);
		$this->Ln();
		$this->SetTextColor(0);
		$this->SetFillColor(255);
		$fontsize=10;
	...
	function Signatures()
		...
		$this->SetTextColor(255);
		$this->SetFillColor(0,102,153);
		$this->Cell(80,6,utf8_to_cp1251(_("Captains' signatures")),'LRTB',0,'C',true);
		$this->Ln();
		
		//home grids
		$this->SetTextColor(0);
		$this->SetFillColor(255);
		$this->Cell(20,8,utf8_to_cp1251(_("Home team")),'LRTB',0,'L',true);
		$this->Cell(60,8,"",'LRTB',0,'L',true);
		...
		$this->SetTextColor(0);
		$this->SetFillColor(255);
		$this->Cell(20,8,utf8_to_cp1251(_("Away")),'LRTB',0,'L',true);
		$this->Cell(60,8,"",'LRTB',0,'L',true);
		$this->Ln();	
		}
	...
	function ScoreGrid()
		...
		$this->SetFillColor(0,102,153);		
		$this->SetX(100);
		$this->Cell(26,4,utf8_to_cp1251(_("Scoring team")),'LRT',0,'C',true);
		$this->Cell(30,4,utf8_to_cp1251(_("Jersey numbers")),'LRT',0,'C',true);
		$this->Ln();
		$this->SetX(100);
		$this->SetFont('Times','',10);
		$this->Cell(13,6,utf8_to_cp1251(_("Home team")),'LRB',0,'C',true);
		$this->Cell(13,6,utf8_to_cp1251(_("Away")),'LRB',0,'C',true);
		$this->Cell(15,6,utf8_to_cp1251(_("Assist")),'LRB',0,'C',true);
		$this->Cell(15,6,utf8_to_cp1251(_("Goal")),'LRB',0,'C',true);
		$this->Cell(22,6,utf8_to_cp1251(_("Time")),'LRTB',0,'C',true);
		$this->Cell(22,6,utf8_to_cp1251(_("Scores")),'LRTB',0,'C',true);
		$this->Ln();
		$this->SetTextColor(0);
		$this->SetFillColor(255);
		for($i=1;$i<41;$i++)
	...
	function FinalScoreTable()
		...
		$this->SetTextColor(255);
		$this->SetFillColor(0,102,153);
		$this->Cell(80,6,utf8_to_cp1251(_("Final score")),'LRTB',0,'C',true);
		$this->Ln();
