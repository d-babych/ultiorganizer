...
if(isset($_POST['save'])) {
	...
	if(empty($html)){
		...
		if($teamId==$game_result['hometeam']){
			header("location:?view=touch/addplayerlists&Game=".$gameId."&Team=".$game_result['visitorteam']);
...
while($player = mysql_fetch_assoc($playerlist))	{
	...
	$html .= "</td></tr><tr><td>\n";
	$html .= "<input class='input' name='p".$player['player_id']."' id='p".$player['player_id']."' maxlength='2' size='2' value='$number'/> ";
