...
//common page
	mobilePageTop(_("Staff&nbsp;interface"));
...
echo "<p>\n";
echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class='groupinglink' href='?'><span class='selgroupinglink'>"._("Home")."</span></a>&nbsp;&nbsp;";
/*if(count($groups>1)){
	foreach($groups as $grouptmp){
		if($group==$grouptmp['reservationgroup']){
			echo "<a class='groupinglink' href='?view=new/respgames&amp;Season=$season&amp;group=".urlencode($grouptmp['reservationgroup'])."'><span class='selgroupinglink'>".U_($grouptmp['reservationgroup'])."</span></a>";
		}else{
			echo "<a class='groupinglink' href='?view=new/respgames&amp;Season=$season&amp;group=".urlencode($grouptmp['reservationgroup'])."'>".U_($grouptmp['reservationgroup'])."</a>";
		}
		echo "&nbsp;&nbsp;&nbsp; ";
	}
	if($group=="all"){
		echo "<a class='groupinglink' href='?view=new/respgames&amp;Season=$season&amp;group=all'><span class='selgroupinglink'>"._("All")."</span></a>";
	}else{
		echo "<a class='groupinglink' href='?view=new/respgames&amp;Season=$season&amp;group=all'>"._("All")."</a>";
	}
...
foreach ($respGameArray as $tournament => $resArray) {
	...
	foreach($resArray as $resId => $gameArray) {		
		...
		if($resId)
			...
			echo $gameArray['locationname'];
		else
			echo _("No location");
		echo "</th>\n<th class='right' colspan='2'>";
		if($resId)
			//echo "<a class='thlink' href='?view=new/pdfscoresheet&amp;Reservation=".$resId."&amp;Season=".$season."'>"._("Print scoresheets")."</a>";
		echo "</th></tr>\n";
		...
		foreach ($gameArray as $gameId => $game) {
			...
			if($game['hometeam'] && $game['visitorteam']){
				if($game['isongoing']){
					echo "<td style='white-space: nowrap' class='right'><a href='?view=new/addresult&amp;Game=".$gameId."'><b>"._("Result")."</b></a> | ";
				} else {
					echo "<td style='white-space: nowrap' class='right'><a href='?view=new/addresult&amp;Game=".$gameId."'>"._("Result")."</a> | ";
				}
				echo "<a href='?view=new/addplayerlists&amp;Game=".$gameId."'>"._("Players")."</a> | ";
				echo "<a href='?view=new/addscoresheet&amp;Game=$gameId'>"._("Score sheet")."</a></td>";
			}
			echo "</tr>";
			echo "<tr><td class='left' colspan='8'></td></tr>";
		}
		echo "</table>";
